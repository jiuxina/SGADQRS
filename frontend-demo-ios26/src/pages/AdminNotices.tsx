...
export default function AdminNotices() {
  ...
  const filtered = mockNotices.filter((n) => {
    ...
    if (filter === 'notice' || filter === 'announcement') return n.type === filter
              ...
              <tbody>
                {sorted.map((notice) => (
                  <tr key={notice.id}>
                    ...
                    <td>
                      <span
                        className={`glass-badge ${notice.type === 'announcement' ? 'reviewing' : 'pending'}`}
                        style={{ fontSize: '11px', padding: '2px 8px' }}
                      >
                        {notice.type === 'notice' ? '通知' : '公告'}
                      </span>
                    </td>
                    <td>
                      <span
                        className={`glass-badge ${notice.status === 'published' ? 'pass' : 'pending'}`}
                        style={{ fontSize: '11px', padding: '2px 8px' }}
                      >
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        ...
                        <button
                          className="text-btn"
                          style={{ fontSize: '12px', display: 'flex', alignItems: 'center', gap: '3px', color: 'var(--danger)' }}
                        >
