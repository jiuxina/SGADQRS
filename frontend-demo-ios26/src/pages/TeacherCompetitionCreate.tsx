        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible">
          ...
          <motion.div style={fieldGroupStyle} variants={staggerItem}>
            <label style={labelStyle}>
              竞赛名称 <span style={{ color: 'var(--danger)' }}>*</span>
            </label>
            <input
          ...
          <motion.div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }} variants={staggerItem}>
            <div>
              <label style={labelStyle}>
                竞赛分类 <span style={{ color: 'var(--danger)' }}>*</span>
              </label>
              <select
                style={selectStyle}
            ...
            <div>
              <label style={labelStyle}>
                主办单位 <span style={{ color: 'var(--danger)' }}>*</span>
              </label>
              <input
                className="glass-input"
                placeholder="请输入主办单位"
          ...
          <motion.div style={fieldGroupStyle} variants={staggerItem}>
            ...
            <textarea
              style={textareaStyle}
              placeholder="请输入竞赛描述，包括竞赛目的、参赛对象等"
          ...
          <motion.div style={fieldGroupStyle} variants={staggerItem}>
            ...
            <textarea
              style={textareaStyle}
              placeholder="请输入竞赛规则，包括参赛要求、评审标准等"
              value={form.rules}
          ...
          <motion.div style={{ marginBottom: '20px' }} variants={staggerItem}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div>
                <label style={{ ...labelStyle, fontSize: '12px', color: 'var(--text-secondary)' }}>
                  报名开始时间 <span style={{ color: 'var(--danger)' }}>*</span>
                </label>
                <input
                  className="glass-input"
                  type="date"
              ...
              <div>
                <label style={{ ...labelStyle, fontSize: '12px', color: 'var(--text-secondary)' }}>
                  报名截止时间 <span style={{ color: 'var(--danger)' }}>*</span>
                </label>
                <input
                  className="glass-input"
                  type="date"
              ...
              <div>
                <label style={{ ...labelStyle, fontSize: '12px', color: 'var(--text-secondary)' }}>
                  比赛开始时间 <span style={{ color: 'var(--danger)' }}>*</span>
                </label>
                <input
                  className="glass-input"
                  type="date"
                  value={form.competitionStart}
                  onChange={(e) => updateField('competitionStart', e.target.value)}
                />
                {errors.competitionStart && <div style={errorStyle}>{errors.competitionStart}</div>}
              </div>
              <div>
                <label style={{ ...labelStyle, fontSize: '12px', color: 'var(--text-secondary)' }}>
                  比赛结束时间 <span style={{ color: 'var(--danger)' }}>*</span>
                </label>
                <input
                  className="glass-input"
                  type="date"
                  value={form.competitionEnd}
                  onChange={(e) => updateField('competitionEnd', e.target.value)}
                />
          ...
          <motion.div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }} variants={staggerItem}>
            ...
            <button className="btn filled-primary" onClick={handleSubmit}>
              <Send size={14} strokeWidth={1.5} /> 提交审核
            </button>
          </motion.div>
        </motion.div>
      </motion.div>
